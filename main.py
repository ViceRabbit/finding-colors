...
allColorsArray = np.array(list(brickColorDict.values()), dtype=np.int32)
...
closestRGB = list(allColorsArray[np.where(differences == np.min(differences))][0].astype(str)) # indexes the color; where.() returns the index of the the least difference. [0] is used to flatten as its [[x]]
